...
export const productoRegistroValidatorResponde = async (
  ...
  setLocale(es)
  const schema = object({
    actividadEconomica: string().required('Debe ingresar el código del producto'),
    ...
    unidadMedida: number().required('Debe seleccionar la unidad de medida'),
...
export const productoRegistroValidator = object({
  ...
  actividadEconomica: mixed().required(),
  precio: number().min(0).required(),
  unidadMedida: mixed().required(),
})
